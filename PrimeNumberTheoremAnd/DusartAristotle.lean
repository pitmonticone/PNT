...
import PrimeNumberTheoremAnd.PrimeInIntervalAristotle
import PrimeNumberTheoremAnd.eSHPAristotle
...
In this section we record the estimates of Dusart [reference] on explicit estimates for sums over primes.
...
/-- **Dusart Proposition 3.2**

For $x \geq e^{b}$ we have $\psi(x) - x| \leq \varepsilon$, where $b, \varepsilon$ are given by [reference].
-/
...
We have $|\psi(x) - x| < \eta_k x / \log^k x$ for $x \geq 2$ with $(k,\eta)$ given by the table in [reference].
-/
...
We have $\vartheta(x)$ and $\psi(x)-\vartheta(x)$ for various $x$ given by [reference], in particular
  $\vartheta(10^{15}) = 999999965752660.939839805291048\dots$
-/
...
We have $|\vartheta(x) - x| < \eta_k x / \log^k x$ for $x \geq x_k$ with $(k,\eta_k,x_k)$ given by the table in [reference].
-/
theorem theorem_4_2 {k : ℕ} {ηk xk : ℝ} (h : (k, ηk, xk) ∈ Table_4_2) {x : ℝ} (hx : x ≥ xk) :
  Eθ x ≤ ηk / (log x) ^ k := by sorry

/-- **Dusart Proposition 4.3**
...
/-- **Dusart Proposition 5.4, substep 1**
...
PROVIDED SOLUTION:
Use Lemma \ref{etheta-pi} and Theorem \ref{Dusart_thm_4_2}.
-/
theorem proposition_5_4a : HasPrimeInInterval.log_thm 4e18 3 := by
  admit

/-- **Dusart Proposition 5.4, substep 2**
...
PROVIDED SOLUTION:
Use Lemma \ref{prime-gap-record-interval} and Proposition \ref{table-8-prime-gap}.
-/
theorem proposition_5_4b (x : ℝ) (hx : x ∈ Set.Ioo 360653 4e18) : HasPrimeInInterval x (x / (log x)^(3:ℝ)) := sorry

/-- **Dusart Proposition 5.4, substep 3**
...
-/
theorem proposition_5_4 : HasPrimeInInterval.log_thm 89693 3 := by
  admit

/-- **Dusart Corollary 5.5**
...
PROVIDED SOLUTION:
Unfortunately, Proposition \ref{Dusart_prop_5_4} only covers the range $x \geq \exp(5000)$.  According to the author, the complete verification of this corollary requires several additional unpublished computations to cover the intermediate range, so this corollary will require significant effort to formalize.
-/
