...
This project request had uuid: 344f7175-266f-462a-995a-9d63caba0fb9
...
/-
Corrected version of S_eq_I with x/n instead of x/n^s.
-/
theorem S_eq_I_corrected (a : ℕ → ℝ) (s x T : ℝ) (hs : s ≠ 1) (hT : 0 < T) :
    let lambda := (2 * π * (s - 1)) / T
    S a s x =
      (x ^ (-s):ℝ) * ∑' (n : ℕ+), a n * (x / n) * I' lambda ((T / (2 * π)) * log (n / x)) := by
        have := @S_eq_I;
        convert this a s x T hs hT using 1;
        contrapose! this;
        refine' ⟨ fun _ => 1, 2, 1, 1, _, _, _ ⟩ <;> norm_num [ CH2.S ];
        rw [ Summable.tsum_eq_zero_add ] <;> norm_num;
        · rw [ ← Equiv.tsum_eq ( Equiv.pnatEquivNat.symm ) ] ; norm_num [ CH2.I' ];
          refine' ne_of_gt ( _ );
          fapply Summable.tsum_lt_tsum;
          use 1;
          · intro n; norm_num [ mul_assoc, mul_comm, mul_left_comm, Real.pi_pos.ne' ];
            split_ifs <;> first | positivity | exact mul_le_of_le_one_right ( by positivity ) ( Real.exp_le_one_iff.mpr <| by linarith [ Real.log_nonneg ( by linarith : ( n : ℝ ) + 1 ≥ 1 ) ] ) ;
          · split_ifs <;> norm_num [ Real.pi_pos.ne' ];
            positivity;
          · refine' Summable.of_nonneg_of_le ( fun n => _ ) ( fun n => _ ) ( summable_nat_add_iff 1 |>.2 <| Real.summable_one_div_nat_pow.2 one_lt_two );
            · positivity;
            · split_ifs <;> norm_num;
              · exact mul_le_of_le_one_right ( by positivity ) ( Real.exp_le_one_iff.mpr ( by nlinarith [ Real.pi_pos, mul_inv_cancel₀ Real.pi_ne_zero, Real.log_nonneg ( by linarith : ( n : ℝ ) + 1 ≥ 1 ) ] ) );
              · positivity;
          · exact_mod_cast summable_nat_add_iff 1 |>.2 <| Real.summable_nat_pow_inv.2 one_lt_two;
        · exact Summable.of_nonneg_of_le ( fun n => by positivity ) ( fun n => by aesop ) ( Real.summable_one_div_nat_pow.2 one_lt_two )

/-
Integral of I' for negative lambda.
-/
lemma CH2.integral_I_prime_neg_lambda {lambda A : ℝ} (hlambda : lambda < 0) (hA : A ≤ 0) :
    ∫ y in Set.Iic A, CH2.I' lambda y = Real.exp (-lambda * A) / -lambda := by
      unfold CH2.I';
      rw [ MeasureTheory.setIntegral_congr_fun measurableSet_Iic fun y hy => if_pos <| by nlinarith [ hy.out ] ];
      have := integral_exp_neg_mul_rpow zero_lt_one ( neg_pos.mpr hlambda );
      rw [ ← neg_neg A, ← integral_comp_neg_Ioi ] ; norm_num at *;
      have h_integral : ∫ x in Set.Ioi (-A), Real.exp (lambda * x) = (∫ x in Set.Ioi 0, Real.exp (lambda * (x - A))) := by
        rw [ ← MeasureTheory.integral_indicator ( measurableSet_Ioi ), ← MeasureTheory.integral_indicator ( measurableSet_Ioi ) ];
        rw [ ← MeasureTheory.integral_sub_right_eq_self _ A ] ; congr ; ext ; rw [ Set.indicator_apply, Set.indicator_apply ] ; aesop;
      simp_all +decide [ mul_sub, Real.rpow_neg_one, div_eq_mul_inv, MeasureTheory.integral_const_mul ];
      simp_all +decide [ sub_eq_add_neg, Real.exp_add, MeasureTheory.integral_mul_const ];
      ring

/-
Bound for the integral term in prop_2_4_plus.
-/
lemma integral_bound {T : ℝ} (hT : 0 < T) {sigma : ℝ} (hsigma : sigma ≠ 1) {x : ℝ} (hx : 1 ≤ x)
    {φ_plus : ℝ → ℂ}
    (hF_int : IntegrableOn (fun y ↦ ‖𝓕 φ_plus y‖) (Set.Iic (-T * log x / (2 * π))))
    (hI_le_Fourier : ∀ y : ℝ, I' ((2 * π * (sigma - 1)) / T) y ≤ ‖𝓕 φ_plus y‖) :
    let lambda := (2 * π * (sigma - 1)) / T
    let A := -T * log x / (2 * π)
    (2 * π * x ^ (1 - sigma) / T) * (-∫ y in Set.Iic A, ‖𝓕 φ_plus y‖) ≤
    if sigma < 1 then -1 / (1 - sigma) else 0 := by
      field_simp;
      split_ifs <;> norm_num at *;
      · have h_integral_bound : ∫ y in Set.Iic (-(T * Real.log x / (2 * Real.pi))), ‖𝓕 φ_plus y‖ ≥ Real.exp (- (2 * Real.pi * (sigma - 1) / T) * (-(T * Real.log x / (2 * Real.pi)))) / -(2 * Real.pi * (sigma - 1) / T) := by
          have h_integral_bound : ∫ y in Set.Iic (-(T * Real.log x / (2 * Real.pi))), CH2.I' (2 * Real.pi * (sigma - 1) / T) y = Real.exp (- (2 * Real.pi * (sigma - 1) / T) * (-(T * Real.log x / (2 * Real.pi)))) / -(2 * Real.pi * (sigma - 1) / T) := by
            convert CH2.integral_I_prime_neg_lambda _ _ using 1;
            · exact div_neg_of_neg_of_pos ( mul_neg_of_pos_of_neg ( by positivity ) ( by linarith ) ) hT;
            · exact neg_nonpos_of_nonneg ( div_nonneg ( mul_nonneg hT.le ( Real.log_nonneg hx ) ) ( by positivity ) );
          refine' h_integral_bound ▸ MeasureTheory.setIntegral_mono_on _ _ _ _ <;> norm_num;
          · contrapose! h_integral_bound;
            rw [ MeasureTheory.integral_undef h_integral_bound ] ; exact ne_of_lt ( div_pos ( Real.exp_pos _ ) ( neg_pos.mpr ( div_neg_of_neg_of_pos ( by nlinarith [ Real.pi_pos ] ) hT ) ) );
          · simpa only [ neg_div ] using hF_int;
          · exact fun y hy => hI_le_Fourier y;
        convert mul_le_mul_of_nonneg_left h_integral_bound ( show 0 ≤ 2 * Real.pi * x ^ ( 1 - sigma ) by positivity ) using 1 ; ring ; ring;
        field_simp;
        rw [ Real.rpow_def_of_pos ( by positivity ) ] ; ring;
        rw [ mul_assoc, ← Real.exp_add, add_comm ] ; norm_num;
      · exact mul_nonneg ( by positivity ) ( MeasureTheory.integral_nonneg fun _ => norm_nonneg _ )
...
theorem prop_2_4_plus {a : ℕ → ℝ} (ha_pos : ∀ n, a n ≥ 0) {T β : ℝ} (hT : 0 < T) (hβ : 1 < β)
  ...
  have := @S_eq_I_corrected ( fun n => if n = 1 then 1 else 0 ) ; norm_num [ Finset.sum_eq_zero, CH2.S ] at this;
  specialize this 0 ( -1 ) 1 ; norm_num at this;
  rw [ eq_comm, tsum_eq_single 1 ] at this <;> norm_num at this;
  · unfold CH2.I' at this ; norm_num at this;
  · aesop
...
theorem F.minus_l1 (lambda y : ℝ) (hlam : lambda ≠ 0) :
    ∫ y : ℝ, I' lambda y - F lambda (-1) y =
      1 / |lambda| - 1 / (rexp (|lambda|) - 1) := by admit
