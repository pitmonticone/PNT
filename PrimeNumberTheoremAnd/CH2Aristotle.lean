...
theorem prop_2_3_1 {a : ℕ → ℂ} {T β : ℝ} (hT : 0 < T) (hβ : 1 < β)
    ...
    (1 / (2 * π)) * ∑' (n : ℕ+), a n * (x / (n ^ sig : ℝ)) * 𝓕 φ ((T / (2 * π)) * log (n / x)) =
      (1 / (2 * π * T)) *
        ∫ t in Set.Icc (-T) T, φ (t/T) * G (sig + t * I) * x ^ (t * I) +
      (∫ y in Set.Iic (-T * log x / (2 * π)), rexp (-y * (sig - 1)) * 𝓕 φ y) * (x ^ (2 - sig) / T : ℝ) := by
      sorry
...
-/
noncomputable def S (a : ℕ → ℝ) (sig x : ℝ) : ℝ :=
  if sig < 1 then ∑ n ∈ Finset.Icc 1 ⌊x⌋₊, a n / (n ^ sig : ℝ)
  else ∑' (n:ℕ), if n ≥ x then a n / (n ^ sig : ℝ) else 0
...
theorem prop_2_4_plus {a : ℕ → ℝ} (ha_pos : ∀ n, a n ≥ 0) {T β : ℝ} (hT : 0 < T) (hβ : 1 < β)
    ...
    (hI_le_Fourier : ∀ y : ℝ, ∀ sig : ℝ, sig ≠ 1 →
      let lambda := (2 * π * (sig - 1)) / T
      I' lambda y ≤ ‖𝓕 φ_plus y‖)
    (x sig : ℝ) (hx : 1 ≤ x) (hsig : sig ≠ 1) :
    S a sig x ≤
      ((2 * π * (x ^ (1 - sig) : ℝ) / T) * φ_plus 0).re +
      (x ^ (-sig) : ℝ) / T *
        (∫ t in Set.Icc (-T) T, φ_plus (t/T) * G (1 + t * I) * (x ^ (1 + t * I))).re -
      if sig < 1 then 1 / (1 - sig) else 0 := by
  sorry
...
theorem prop_2_4_minus {a : ℕ → ℝ} (ha_pos : ∀ n, a n ≥ 0) {T β : ℝ} (hT : 0 < T) (hβ : 1 < β)
    ...
    (hFourier_le_I : ∀ y : ℝ, ∀ sig : ℝ, sig ≠ 1 →
      let lambda := (2 * π * (sig - 1)) / T
      ‖𝓕 φ_minus y‖ ≤ I' lambda y)
    (x sig : ℝ) (hx : 1 ≤ x) (hsig : sig ≠ 1) :
    S a sig x ≥
      ((2 * π * (x ^ (1 - sig) : ℝ) / T) * φ_minus 0).re +
      (x ^ (-sig) : ℝ) / T *
